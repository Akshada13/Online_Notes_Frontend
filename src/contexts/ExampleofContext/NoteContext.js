...
export default function NoteContext(props) {
    ...
    async function fetchnotes(authtoken) {
        let response = await axios.get('https://online-notes13.herokuapp.com/api/notes/fetchallnotes', {
            headers: {
    ...
    async function addnote(authtoken, Title, description,Tags) {
        let response = await axios.post('https://online-notes13.herokuapp.com/api/notes/insertnote', { Title, description,Tags }, {
            headers: {
                'auth-token': authtoken
            }
        })
    ...
    async function deletenote(id, authtoken) {
        let response = await axios.delete(`https://online-notes13.herokuapp.com/api/notes/deletenote/${id}`, {
            headers: {
                'auth-token': authtoken
            }
        })
    ...
    async function updatenotes(id,Title,description,Tags,authtoken) {
        let response= await axios.put(`https://online-notes13.herokuapp.com/api/notes/updatenote/${id}`,{Title,description,Tags},{
            headers: {
                'auth-token': authtoken
            }
        })
    ...
    async function filternote(tag, authtoken) {
        let response = await axios.get(`https://online-notes13.herokuapp.com/api/notes/filternotes/${tag}`, {
            headers: {
                'auth-token': authtoken
            }
        })
